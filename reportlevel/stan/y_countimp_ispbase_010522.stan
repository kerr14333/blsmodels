...
transformed parameters {
  ...
  for( k in 1:n_2 )
  {
    lambda_2[k] = delta[2-1] * lambda_1[second_order[k,1]] * lambda_1[second_order[k,2]];
  } /* end loop k over n_2 second order interaction random effects */
  
  for( k in 1:n_3 )
  {
    lambda_3[k] = delta[3-1] * lambda_1[third_order[k,1]] * lambda_1[third_order[k,2]] 
                                                          * lambda_1[third_order[k,3]];
  } /* end loop k over n_2 second order interaction random effects */
